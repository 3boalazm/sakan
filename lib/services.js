...
export async function listMyJourney(s, limit = 60) {
  // جيب كل events بتاعت الـ pair مرتبة من الأحدث
  let snap;
  try {
    snap = await C('events')
    ...
    // لو مش عنده index يرجع كل الـ events بدون ترتيب
    snap = await C('events')
      .where('pairId', '==', s.pairId)
      .get();
  }

  // جيب كل الـ resources عشان نعرف عناوينها
